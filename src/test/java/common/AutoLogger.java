...
public class AutoLogger implements WebDriverEventListener {
	...
	/**
	 * Description - Method returns field object which is used for a webelement in a
	 * page class.
	 */
	...
	public void afterChangeValueOf(WebElement element, WebDriver arg1, CharSequence[] arg2) {
		...
		if (field != null) {
			...
			if (arg2 != null) {
				System.out.println("Entered in '" + element.toString().split("->")[1].trim() + "'");
				logs = logs + "\n " + "Entered in '" + element.toString().split("->")[1].trim() + "'";
			} else {
